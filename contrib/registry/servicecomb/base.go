// Package servicecomb 提供 ServiceComb 服务注册中心的基础能力。
//
// 本文件内联了 baseregistry / native / lifecycle 的公共逻辑，使 servicecomb 包成为完全独立的模块，
// 不再依赖 contrib/internal 或 contrib/registry/internal 下的任何包。
package servicecomb
...
// ============================================================================
// 心跳续租生命周期（内联自 contrib/registry/internal/lifecycle）
// ============================================================================

// HeartbeatLoopConfig 定义心跳循环的配置参数。
//
// 各注册中心（如 Eureka、ServiceComb）使用此配置驱动心跳续租逻辑。
type HeartbeatLoopConfig struct {
	// Interval 心跳发送间隔
	Interval time.Duration
	// RetryBackoff 心跳失败后的退避等待时长
	RetryBackoff time.Duration
	// Heartbeat 执行一次心跳的回调函数
	Heartbeat func(context.Context) error
	// Recover 心跳失败后执行恢复注册的回调函数
	Recover func(context.Context) error
	// ShouldRecover 判断心跳错误是否需要触发恢复注册
	ShouldRecover func(error) bool
}

// RunHeartbeatLoop 运行心跳续租循环。
//
// 循环行为：
//   - 按 Interval 间隔定期调用 Heartbeat
//   - 心跳成功则继续下一次循环
//   - 心跳失败且 ShouldRecover 返回 true，则尝试 Recover 恢复注册
//   - 恢复成功则继续下一次循环
//   - 恢复失败或 ShouldRecover 返回 false，则按 RetryBackoff 退避后继续
//   - ctx 取消时退出循环
