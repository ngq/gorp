...
// Success 通过当前 responder 链路输出成功响应。
//
// Example:
//
...
// SuccessWithMessage 输出带自定义 message 的成功响应。
...
// BadRequest 通过当前 responder 链路输出请求错误响应。
...
// InternalError 通过当前 responder 链路输出内部错误响应。
...
// responderFor 从 context 解析 responder，失败时回退到默认 responder。
