...
// - Expose the framework logging capability through a short top-level package.
// - Re-export common logger helpers so business code does not need to import the internal framework/log package directly.
// - Provide convenient field builders and context-related logger accessors.
//
// 适用场景：
// - 通过简短的顶层包暴露框架日志能力。
// - 重新导出常用 logger helper，让业务代码不必直接 import 内部的 framework/log 包。
// - 提供便捷的字段构造器和 context 相关 logger 访问入口。
package log
