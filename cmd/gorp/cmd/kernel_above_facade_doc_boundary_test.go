...
func TestKernelAboveFacadePlanDeclaresTemplateAcceptanceState(t *testing.T) {
	content, err := os.ReadFile("E:/project/gin_plantfrom/.private-docs/manual/v1-prelude/gorp-platform-kernel-above-facade-实施方案.zh-CN.md")
	require.NoError(t, err)
	text := string(content)

	require.Contains(t, text, "当前模板侧验收状态：")
	require.Contains(t, text, "`golayout`、`multi-flat-wire`、`multi-independent` 已按该顺序完成整体重写与封板")
	require.Contains(t, text, "默认入口已收敛到 facade-first 主线")
