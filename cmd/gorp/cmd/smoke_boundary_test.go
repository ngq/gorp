...
func TestSmokeScriptExistsAndCoversCurrentMainline(t *testing.T) {
	...
	require.Contains(t, text, "go test -race ./framework/container/...")
	require.Contains(t, text, "go test -race ./framework/contract/...")
	require.Contains(t, text, "go test -race ./framework/provider/...")
	require.Contains(t, text, "go test -race ./framework/bootstrap/...")
	require.Contains(t, text, "go test -race ./contrib/...")
