...
func goTypeFromCols(cols []datacontract.Column, name, driver string) string {
	for _, c := range cols {
		if c.Name == name {
			return dbTypeToGo(driver, c.Type)
		}
	}
	return "int64"
}
...
{{- range .Cols }}
	{{- if .Comment }}// {{ .Comment }}
	{{ toGoName .Name }} {{ goType $.Driver .Type }} ` + "`" + `gorm:"column:{{.Name}}{{if .PrimaryKey}};primaryKey{{end}}" db:"{{.Name}}" json:"{{.Name}}"` + "`" + `
{{- else }}// {{ toGoName .Name }}
	{{ toGoName .Name }} {{ goType $.Driver .Type }} ` + "`" + `gorm:"column:{{.Name}}{{if .PrimaryKey}};primaryKey{{end}}" db:"{{.Name}}" json:"{{.Name}}"` + "`" + `
{{- end }}
{{- end }}
...
{{- range .Cols }}
	{{- if .Comment }}// {{ .Comment }}
	{{ toGoName .Name }} {{ goType $.Driver .Type }} ` + "`db:\"{{.Name}}\" json:\"{{.Name}}\"`" + `
{{- else }}// {{ toGoName .Name }}
	{{ toGoName .Name }} {{ goType $.Driver .Type }} ` + "`db:\"{{.Name}}\" json:\"{{.Name}}\"`" + `
{{- end }}
{{- end }}
...
func Create{{.Name}}(ctx context.Context, client *ent.Client, fields map[string]any) (*model.{{.Name}}, error) {
	...
	if v, ok := fields["{{.Name}}"]; ok {
		builder = builder.Set{{ toGoName .Name }}(v.({{ goType $.Driver .Type }}))
	}
	{{- end }}
	{{- end }}
