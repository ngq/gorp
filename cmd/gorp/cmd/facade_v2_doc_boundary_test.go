...
func TestFacadeV2DocDeclaresFacadeFirstUserLayering(t *testing.T) {
	content, err := os.ReadFile("E:/project/gin_plantfrom/.private-docs/manual/v2/facade/gorp-platform-kernel-above-facade-复杂能力定义与简化路径.zh-CN.md")
	require.NoError(t, err)
	text := string(content)

	require.Contains(t, text, "## 5.4 用户分层使用口径")
	require.Contains(t, text, "facade 是统一主入口，面向普通用户，也面向大多数高级用户；kernel 是扩展与回退层，不是高级用户默认入口。")
	require.Contains(t, text, "Kratos 不是“复杂场景才使用的 facade”，而是“默认高层入口 + 允许复杂场景下探底层”的框架。")
	require.Contains(t, text, "普通用户和大多数高级用户，都先走 facade-first 默认路径")
	...
	require.Contains(t, text, "kernel capability -> facade declaration -> starter consumption")
...
func TestFacadeV2ChecklistKeepsExternalReferenceBoundary(t *testing.T) {
	content, err := os.ReadFile("E:/project/gin_plantfrom/.private-docs/manual/v2/facade/gorp-platform-kernel-above-facade-P0-执行checklist（按文件级）.zh-CN.md")
	require.NoError(t, err)
	text := string(content)
...
func TestRuntimeV2DocDeclaresHostLifecycleBoundary(t *testing.T) {
	content, err := os.ReadFile("E:/project/gin_plantfrom/.private-docs/manual/v2/facade/gorp-runtime-定义清单与语义说明.zh-CN.md")
	require.NoError(t, err)
	text := string(content)
	...
	require.Contains(t, text, "HTTP 已经消费它做优雅关闭；Cron / gRPC 具备接入适配器，但不等于默认由 facade 托管")
