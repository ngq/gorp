...
//
// 注意：ginContext 不再实现 context.Context（不提供 Value/Done/Deadline/Err），
// 调用方必须通过 c.Context() 显式获取标准 context.Context。
// 这是为了避免 ginContext 被 context 链双向引用导致栈溢出。BUG-001。
...
// 必须通过此方法获取标准 context，不能把 ginContext 直接当 context 用。
...
func (c *ginContext) BindURI(obj any) error    { return c.gin.ShouldBindUri(obj) }
func (c *ginContext) BindHeader(obj any) error { return c.gin.ShouldBindHeader(obj) }
func (c *ginContext) BindForm(obj any) error   { return c.gin.ShouldBind(obj) }
...
func (c *ginContext) File(path string)                     { c.gin.File(path) }
func (c *ginContext) FileAttachment(path, filename string) { c.gin.FileAttachment(path, filename) }
...
func (c *ginContext) Stream(contentType string, write func(transportcontract.StreamWriter) error) error {
	...
	return write(ginStreamWriter{writer: c.gin.Writer})
}
...
type ginStreamWriter struct{ writer gin.ResponseWriter }

func (w ginStreamWriter) Write(data []byte) (int, error) { return w.writer.Write(data) }
func (w ginStreamWriter) Flush() error                   { w.writer.Flush(); return nil }
