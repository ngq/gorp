...
func main() {
	if err := bootstrap.BootHTTPService("store-service", bootstrap.Options{}, migrate, setup); err != nil {
...
func setup(rt *bootstrap.HTTPServiceRuntime) error {
	storeService, err := wireStoreService(rt.DB, bootstrap.MustMakeJWTService(rt.Container))
	if err != nil {
		return err
	}

	storeService.RegisterRoutes(rt.Engine)
	return nil
}
