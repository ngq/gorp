...
### 能力位与默认实现（不等于全部都属于 framework 核心）
...
| Wire 依赖注入 | ✅ golayout-wire / multi-flat-wire 模板 | ❌ 无说明文档 | 需补充 |
| 多服务项目 | ✅ multi-flat-wire（主推）/ multi-independent（进阶） 模板 | ❌ 无说明文档 | 需补充 |
...
**优势**：
- Provider 体系完善（能力位覆盖较全，但核心层与具体后端边界仍需继续收薄）
- 微服务能力完整（服务发现、熔断、追踪等能力位完整）
- noop/真实实现双模式（单体→微服务平滑演进）
- 工具链齐全（Proto 三工作流、Model 生成、脚手架）
- 公开主线已基本收敛为：`golayout` 默认单服务、`multi-flat-wire` 主推微服务、`multi-independent` 进阶多服务
