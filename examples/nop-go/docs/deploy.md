...
说明：
- compose 容器名使用 `nop-go-*`
- 本地镜像名使用 `nop-go-user-service`、`nop-go-order-service`、`nop-go-product-service`
...
- `nop-go-user-service`
- `nop-go-order-service`
- `nop-go-product-service`
...
- `deploy/kubernetes/overlays/staging/kustomization.yaml` 中的镜像仓库与 tag（镜像名已默认对齐 `nop-go-*`）
- `deploy/kubernetes/overlays/prod/kustomization.yaml` 中的镜像仓库与 tag（镜像名已默认对齐 `nop-go-*`）
