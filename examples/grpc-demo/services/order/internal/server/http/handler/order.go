...
func (h *OrderHandler) List(c gorp.Context) {
	...
	items, total, err := h.order.List(c, page, size)
	if err != nil {
		gorp.Error(c, err)
		return
	}
...
func (h *OrderHandler) GetByID(c gorp.Context) {
	...
	order, err := h.order.GetByID(c, uint(id))
	if err != nil {
...
func (h *OrderHandler) Create(c gorp.Context) {
	...
	order, err := h.order.Create(c, service.CreateOrderRequest{
		UserID:      req.UserID,
...
func (h *OrderHandler) Delete(c gorp.Context) {
	...
	if err := h.order.Delete(c, uint(id)); err != nil {
		gorp.Error(c, err)
		return
	}
...
func (h *OrderHandler) GetOrderUser(c gorp.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		gorp.BadRequest(c, "invalid id")
		return
	...
	resp, err := h.order.GetOrderUser(c, uint(id))
	if err != nil {
...
func (h *OrderHandler) LockOrderDemo(c gorp.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		gorp.BadRequest(c, "invalid id")
		return
	...
	resp, err := h.order.LockOrderDemo(c, uint(id))
	if err != nil {
