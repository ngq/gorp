...
## Docker 开发部署
- 构建镜像：`docker build -t admin:latest .`
...
- `deploy/kubernetes/overlays/staging/kustomization.yaml` 中的镜像仓库与 tag（镜像名默认对齐 `admin`）

默认资源命名：

- ConfigMap：`admin-config`
- Secret：`admin-secret`
- Namespace（dev / staging）：`admin-dev` / `admin-staging`
- 镜像名：`admin`
